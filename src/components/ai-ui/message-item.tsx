...
const styles = {
  ...
  message: (isUser: boolean) => ({
    ...
    wrapper: cn("max-w-[85%] relative", !isUser && "bg-muted rounded-2xl px-6 py-4"),
    text: cn(
      "text-right w-full text-[13px] leading-[1.8]",
      isUser ? "whitespace-pre-line" : "whitespace-normal",
      "break-words"
    ),
    footer: cn("flex items-end justify-between mt-3", isUser && "flex-row-reverse"),
    time: cn(
  ...
  button: (copied: boolean, isAnimating: boolean) => ({
    ...
    icon: {
      wrapper: (visible: boolean) => cn(
        "absolute inset-0 flex items-center justify-center transition-all duration-200",
        visible ? "scale-100 rotate-0 opacity-100" : "scale-0 rotate-[180deg] opacity-0"
      ),
      ring: cn(
        "absolute inset-[-4px] rounded-full border-2 transition-all duration-500",
        isAnimating ? "border-green-500 scale-110 opacity-0" : "border-transparent scale-100 opacity-100"
      ),
    },
  }),
} as const;
...
/* --------------------------------- Utilities --------------------------------- */
const formatTime = (date: Date) => date.toLocaleTimeString(TIME.locale, TIME.format);
...
/* --------------------------------- Components --------------------------------- */
const ActionButton = ({ copied, isAnimating, onClick, tooltipText, icon: Icon }: ActionButtonProps) => (
  <Tooltip>
    <TooltipTrigger asChild>
      <Button
        ...
        onClick={onClick}
      >
        {isAnimating && <div className={styles.button(!!copied, !!isAnimating).icon.ring} />}
        <div className={styles.button(!!copied, !!isAnimating).icon.wrapper(!copied)}>
...
const MessageActions = ({ isUser, content, onRetry }: Pick<MessageItemProps, 'onRetry'> & { isUser: boolean; content: string }) => {
  ...
  const handleCopy = async () => {
    ...
    if (success) {
      setCopied(true);
      setIsAnimating(true);
      setTimeout(() => setIsAnimating(false), TIME.animation.copy);
      setTimeout(() => setCopied(false), TIME.animation.reset);
    }
  };
...
const MessageContent = ({ content, isUser, isStreaming }: { content: string; isUser: boolean; isStreaming?: boolean }) => {
  const CodeBlockComponent = useMemo(() => {
    const Component = (props: CodeProps) => <CodeBlock {...props} isUser={isUser} />;
    ...
    <>
      <ReactMarkdown
        ...
        components={{
          ...
          p: ({ children }) => (
            <p className={cn(styles.base.text, !isUser && "whitespace-pre-wrap")}>
              {children}
            </p>
          ),
        }}
        ...
        <div className={messageStyles.wrapper}>
          <div className={messageStyles.text}>
            ...
            <div className={messageStyles.footer}>
              ...
              />
              {createdAt && (
                <time dateTime={createdAt.toISOString()} className={messageStyles.time}>
